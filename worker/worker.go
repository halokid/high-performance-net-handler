...
func (w Worker) Start() {
  ...
  go func() {
    ...
    for {
      ...
      // fixme: 逻辑上应该是这样, 这个 for 监听 channel 的用户， 实际上不能是死循环， 应该是有一个循环的大小的， 那么
      // fixme:  w.work_pool <- w.job_channel 就是这个循环的大小， 这一句的意义是定义这个 for循环几次用的。。
      // fixme: 因为  w.work_pool 的大小是3， 所以这个 for 循环每次就是 循环 3 次， 每次处理 3 个 job
